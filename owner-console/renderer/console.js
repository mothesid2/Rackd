...
async function renderBusinessDetail() {
          ...
          <div class="rowflex" style="justify-content:space-between">
            ...
            ${isAdmin ? '' : isCashier
              ...
              : `<span class="rowflex" style="gap:6px">
                   ...
                   <button class="rowbtn" data-promote="${esc(e.uid)}" data-who="${esc(e.username || e.name || 'this user')}">Promote to admin</button>
                 </span>`}
          </div>
  ...
  $('usersWrap').querySelectorAll('[data-promote]').forEach((b) => b.addEventListener('click', async () => {
    if (!confirm(`Promote ${b.dataset.who} to admin? Admins get every permission and can't be permission-limited afterward.`)) return;
    b.disabled = true; b.textContent = 'Promoting…';
    const r = await window.owner.promoteToAdmin(currentTenant, b.dataset.promote);
    if (!r.success) { b.disabled = false; b.textContent = 'Promote to admin'; toast(r.error || 'Failed', true); return; }
    toast(`${b.dataset.who} is now an admin`);
