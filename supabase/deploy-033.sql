-- Rackd cloud schema — INCREMENTAL bundle: storefront menu RPC (Phase 3).
-- Apply after deploy-032.sql. Public (anon) menu for storefront-enabled locations.

-- ==================== supabase/migrations/033_storefront_menu.sql ====================
-- 033_storefront_menu.sql
-- Public menu for the storefront. inventory_cloud is NOT anon-readable (tenant RLS),
-- so this SECURITY DEFINER function exposes ONLY opted-in (is_visible) products for
-- storefront-ENABLED locations, with live-ish available stock (snapshot minus
-- reserved). The storefront polls this; oversell is still enforced at reserve time.
