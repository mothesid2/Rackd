// Supabase Edge Function: rebate-submit
//
// Weekly manufacturer scan-data submission. Invoked by pg_cron (daily) in
// dispatch mode: it finds every (tenant, manufacturer) whose report is due,
// aggregates the period's applied rebates, runs the manufacturer's pluggable
// formatter, uploads the file via SFTP, and logs the attempt to
// manufacturer_submissions. Retries a failed period on each subsequent run until
// it succeeds or the period rolls off. Can also be called explicitly for one job.
//
// POST { secret, mode: 'dispatch' }
//   or { secret, tenant_id, manufacturer_uid, period_start?, period_end? }
//
// Env: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY (auto), REBATE_DISPATCH_SECRET
//
...
Deno.serve(async (req: Request) => {
  ...
  if (body?.manufacturer_uid && body?.tenant_id) {
    ...
    for (const m of mfrs ?? []) {
      ...
      if (done) continue; // already submitted this period
      jobs.push({ mfr: m, period });
    }
  }
...
async function processJob(admin: any, mfr: any, period: { start: string; end: string; dueDate: string }) {
  ...
  // Reuse a prior non-success attempt for this period (retry) or open a new one.
