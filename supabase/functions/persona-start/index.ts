// persona-start — creates a Persona verification inquiry bound to the AUTHENTICATED
// customer (reference-id = auth.uid()), so the webhook can only ever verify the
// right account. Returns a one-time hosted link for the storefront to open.
...
Deno.serve(async (req: Request) => {
  ...
  // Create the inquiry with reference-id bound to this user.
  ...
  // One-time hosted link the storefront redirects to.
