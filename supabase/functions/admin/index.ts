  ...
  try {
    switch (action) {
      ...
      case 'publish': {
        ...
        if (lErr) throw lErr;
        const real = (files ?? []).filter((f: { name: string; id: string | null }) => f.id !== null); 
      ...
      case 'setStaffRole': {
        ...
        if (!tenantId || !employeeUid || role !== 'admin') return json({ error: 'tenant_id + employee_uid + role="admin" required' }, 400);
        const { data: emp } = await admin.from('employees_cloud')
          .select('username, name, role').eq('tenant_id', tenantId).eq('uid', employeeUid).maybeSingle();
        if (!emp) return json({ error: 'employee not found' }, 404);
        if (emp.role !== 'manager') return json({ error: 'Only a manager can be promoted to admin.' }, 400);
        const { error } = await admin.from('employees_cloud')
          .update({ role: 'admin', updated_at: new Date().toISOString() })
          .eq('tenant_id', tenantId).eq('uid', employeeUid);
        if (error) throw error;
        await admin.from('owner_audit_log').insert({
          tenant_id: tenantId, action: 'promote_to_admin', target_uid: employeeUid,
          target_label: emp.username || emp.name, detail: { from_role: emp.role },
        });
        return json({ ok: true });
      }
      ...
      case 'mfrCredsSet': {
        ...
        if (vaultId) row.vault_secret_id = vaultId; 
