// Supabase Edge Function: billing (spec v2 §6)
//
// Owner-initiated billing actions, gated by the shared ADMIN_SECRET (same gate as
// the `admin` function). Holds the Stripe SECRET key server-side; the POS never
// sees it. The billing engine itself (dunning, status transitions) is driven by
// Stripe + the `stripe-webhook` function — this function only starts flows.
//
// POST { admin_secret, action, ...payload }
//   action: 'checkout' | 'portal' | 'status'
//
// Function env (set via `supabase secrets set ...`):
//   ADMIN_SECRET, STRIPE_SECRET_KEY,
//   STRIPE_PRICE_CORE_MONTHLY, STRIPE_PRICE_CORE_ANNUAL,
//   STRIPE_PRICE_STANDARD_MONTHLY, STRIPE_PRICE_STANDARD_ANNUAL,
//   STRIPE_PRICE_PRO_MONTHLY, STRIPE_PRICE_PRO_ANNUAL,
//   BILLING_SUCCESS_URL (optional), BILLING_CANCEL_URL (optional)
//
// PROCESSOR-AGNOSTIC SEAM: every Stripe call goes through the `stripe` client
// below. To swap Stripe for Paddle/etc., replace this one client + the price map;
// the action handlers and the `licenses` billing columns stay the same.
    ...
    switch (action) {
      ...
      case 'checkout': {
        ...
        const session = await stripe.checkout.sessions.create({
          ...
          // 14-day trial, card required at signup (spec §6).
