// Supabase Edge Function: stripe-webhook (spec v2 §6)
//
// The billing engine. Stripe posts subscription lifecycle events here; we verify
// the signature and project them onto the `licenses` row. Setting licenses.active
// = false is what puts the POS into read-only (via licenseCheck.ts) on suspend.
//
// Dunning is Stripe's job (Smart Retries — configure day 3/7/14 in the Stripe
// dashboard's revenue-recovery settings). On final failure Stripe moves the sub to
// `unpaid`/`canceled`, which we map to suspended here.
//
// Function env: STRIPE_SECRET_KEY, STRIPE_WEBHOOK_SECRET (auto SUPABASE_URL/SERVICE_ROLE).
// Configure in Stripe as a webhook to: <project>/functions/v1/stripe-webhook
...
// Map a Stripe subscription.status to our status + whether the POS stays enabled.
function projectStatus(s: string, graceUntil: string | null): { subscription_status: string; active: boolean } {
  switch (s) {
    ...
    case 'past_due': {
      // Stay live during the 7-day grace window, then restrict.
  ...
  try {
    switch (event.type) {
      ...
      case 'customer.subscription.updated': {
        ...
        // Read existing grace_until so past_due grace persists across updates.
      ...
      case 'invoice.payment_failed': {
        ...
        // Enter past_due with a 7-day grace before the POS is restricted.
        await updateByCustomer(customerId, {
      ...
      case 'customer.subscription.deleted': {
        const sub = event.data.object as Stripe.Subscription;
        const customerId = String(sub.customer);
        // Cancelled: POS read-only, retain data 90 days then flag for deletion.
        await updateByCustomer(customerId, {
