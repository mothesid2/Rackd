-- Rackd cloud schema — INCREMENTAL bundle: rebate dashboard summary (Part 5).
-- Apply after deploy-026.sql.

-- ==================== supabase/migrations/027_rebate_summary.sql ====================
-- 027_rebate_summary.sql
-- Part 5: manager rebate dashboard aggregate. Per manufacturer for a period:
-- applied (auto vs manual) + $, missed opportunities + estimated $, and the last
-- successful submission. SECURITY INVOKER so RLS scopes it — a manager token
-- (tenant-wide) sees the whole business.
