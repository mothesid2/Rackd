  ...
  items as (
    select ti.*
     ...
     where ti.tenant_id = p_tenant and ti.location_id = p_location
       and ti.created_at >= p_start and ti.created_at < p_end
  ),
  summary as (
    select
...
create or replace function public.location_period_report_window(p_start timestamptz, p_end timestamptz)
returns jsonb language sql stable security invoker set search_path = public as $$
  select public.location_report_core_window(public.current_tenant_id(), public.current_location_id(), p_start, p_end);
$$;
grant execute on function public.location_period_report_window(timestamptz, timestamptz) to authenticated;
