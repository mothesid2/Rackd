-- Rackd cloud schema — INCREMENTAL bundle: storefront manager control (Phase 4).
-- Apply after deploy-033.sql. Adds set_storefront_settings() so a MANAGER token
-- can flip a location's storefront on/off + tax rate (locations is otherwise
-- service-role-write-only). Menu opt-in + order-queue writes reuse existing
-- storefront_products / online_orders manager policies — no new grants needed.

-- ==================== supabase/migrations/034_storefront_manager.sql ====================
