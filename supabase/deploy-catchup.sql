...
-- ############################## deploy-040 ##############################
-- Rackd cloud schema — deploy bundle for migration 040 (Stripe Connect + 5% online fee).
-- Idempotent: safe to re-run. Paste into the Supabase SQL editor and Run.

-- ==================== supabase/migrations/040_stripe_connect_fee.sql ====================
...
-- ############################## deploy-042 ##############################
-- Rackd cloud schema — deploy bundle for migration 042 (product photos).
-- Idempotent: safe to re-run. Paste into the Supabase SQL editor and Run.

-- ==================== supabase/migrations/042_product_images.sql ====================
...
-- ############################## deploy-045 ##############################
-- Rackd cloud schema — deploy bundle for migration 045 (online order customer name/phone).
-- Idempotent: safe to re-run. Paste into the Supabase SQL editor and Run.

-- ==================== supabase/migrations/045_online_order_customer_name.sql ====================
-- 045_online_order_customer_name.sql
--
-- The auto-printed pickup ticket needs the customer's NAME (and phone) so counter
-- staff can match the bag to the person. The register prints via its license JWT,
-- whose RLS cannot read `storefront_customers` (customers read only their own row).
-- So denormalize name+phone onto the order at insert time with a SECURITY DEFINER
-- trigger — the order row is already readable by store staff, so the ticket has it.
...
-- Backfill any orders that predate this trigger.
...
-- ############################## deploy-046 ##############################
-- Rackd cloud schema — deploy bundle for migration 046 (owner console expansion).
-- Idempotent: safe to re-run. Paste into the Supabase SQL editor and Run.

-- ==================== supabase/migrations/046_owner_console_expansion.sql ====================
