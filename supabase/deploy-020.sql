-- Rackd cloud schema — INCREMENTAL bundle: Phase 4 (X/Z location roll-up).
-- Apply after deploy-019.sql. Idempotent; paste into the Supabase SQL editor and Run.

-- ==================== supabase/migrations/020_location_period_report.sql ====================
-- 020_location_period_report.sql
-- X/Z location roll-up (spec v3, Phase 4). Aggregates the cloud sales mirrors for
-- the CALLER'S location (all its registers) over a date range, returning the same
-- JSON shape as the local buildPeriodReport() so the X/Z screen can render either
-- "This register" (local SQLite) or "This location" (this RPC) with one code path.
--
-- SECURITY INVOKER: runs as the caller, so current_tenant_id()/current_location_id()
-- come from the register's JWT and RLS still applies. Dates are compared in the
-- store timezone (the app is America/Chicago throughout) to match local-day rollup.
