/*
 * Multi-location (v3) end-to-end verification.
 *
 * Confirms the Phase 0/1 cloud changes are live:
 *   1) jwt-issuer mints a token carrying location_id + register_id
 *   2) the caller's license row has a backfilled location_id
 *   3) a locations row exists for that location (015 backfill)
 *   4) location-scoped RLS reads work (customers_cloud select doesn't error)
 *
 * Prereqs:
 *   - cloud migrations through 017 applied (deploy-015-017.sql)
 *   - jwt-issuer Edge Function redeployed (emits location_id/register_id)
 *   - .env has SUPABASE_URL, SUPABASE_ANON_KEY, TEST_LICENSE_KEY
 *
 * Run:  npx tsc scripts/verify-location.ts --outDir .tmp --module commonjs \
 *          --target ES2020 --esModuleInterop --skipLibCheck && node .tmp/verify-location.js
 */
import { createClient } from '@supabase/supabase-js';
import fs from 'fs';

// minimal .env loader (same as auth-demo.ts)
...
const MACHINE_ID = 'verify-location-script'; // stable fake register id for the claim
...
async function main() {
  ...
  // 2) own license row has a location_id
  ...
  // 3) a locations row exists and is readable
  ...
  // 4) location-scoped read works (no error, no leak)
