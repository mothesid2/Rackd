...
function run() {
  // Derive the same userData path Electron uses at runtime
  // Windows: %APPDATA%\sei-vape-pos   Mac/Linux: ~/Library/... or ~/.config/...
  ...
  // Seed products — a vertical-neutral sample mix (vape, smoke, adult, general
  // retail) showing both age-restricted and unrestricted items. age_restricted
  // is set per item; the store's minimum-age policy lives in Settings.
  const products = [
    // Vape
    ...
    // General retail (not age-restricted)
