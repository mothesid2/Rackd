...
/**
 * Auth model correction (batch 3, item 1). Three structural changes the old
 * `users` table can't express, so it's rebuilt (SQLite can't ALTER a CHECK or
 * drop NOT NULL):
 *
 *   • role gains 'admin' — the business admin (provisioned at onboarding, manages
 *     staff). CHECK becomes admin|manager|cashier.
 *   • username + password_hash become NULLABLE — cashiers have neither (PIN only).
 *   • must_change_pin — forced PIN change on first login (managers + cashiers),
 *     alongside the existing must_change_password (managers + admin).
 *
 * Idempotent: skips the rebuild if the table already has both the 'admin' CHECK
 * and the must_change_pin column.
 */
export const migration013: Migration = {
  ...
  up(db: Database.Database) {
    ...
    if (hasPin && allowsAdmin) return; // already migrated
