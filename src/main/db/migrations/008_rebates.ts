...
/**
 * Manufacturer rebate / scan-data programs (Altria, RJR, ITG) — local schema.
 *
 * Data flows two ways, consistent with Rackd's local-first model:
 *   • manufacturers + rebate_rules  — CONFIG, authored in the cloud (manager),
 *     TENANT-scoped, pulled DOWN so every register can detect rebates offline.
 *   • applied_rebates + missed_rebates — AUDIT, written locally at checkout,
 *     LOCATION+REGISTER scoped, pushed UP; they feed the weekly cloud report.
 *
 * Identity is a `uid` (uuid) on every row so it survives cross-register + cloud
 * sync (same pattern as stock_movements / customers). SFTP credentials and the
 * submission log live ONLY in the cloud — never on a register.
 */
export const migration008: Migration = {
  ...
  up(db: Database.Database) {
    // Link products to a manufacturer (nullable; set via catalog / SKU mapping).
    ...
    // Seed the three known scan-data manufacturers with FIXED uids so rules
    // reference stable ids across registers + cloud, and the report cadence is
    // preset. Cloud provisioning (SFTP creds, per-tenant rows) reuses these uids.
    //   Altria: batch Sun–Sat (ends Sat=6), due Tue=2 the following week.
    //   RJR:    batch Mon–Sun (ends Sun=0), due Wed=3 the following week.
    //   ITG:    cadence TBD during certification (left null / configurable).
  ...
  down(db: Database.Database) {
    ...
    `);
    // products.manufacturer_uid left in place (SQLite can't drop columns pre-3.35).
  },
};
