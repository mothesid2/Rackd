...
/**
 * Tip-pool audit trail (batch 5 legal rework). Persisted once per business day
 * at Z-report close (not on every X-report preview, which is just a live
 * calculation) so the fee rate actually used and the computed deduction are
 * reconstructible later — not just the final per-employee numbers. `pooled=0`
 * rows are the 29 CFR 531.52 sole-service exception: no eligible (cashier)
 * employee worked, so tips were never pooled — `tip_pool_shares` for that
 * ledger row holds each employee's DIRECT tip total instead of a pool share.
 */
