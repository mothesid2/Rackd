...
function getDbPath(): string {
  try {
    ...
    // Not running in Electron (e.g. seed script)
  }
...
export function initSchema(db: Database.Database): void {
  ...
  try { db.exec(`ALTER TABLE invoices ADD COLUMN pdf_path TEXT`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE users ADD COLUMN must_change_password INTEGER DEFAULT 0`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE transactions ADD COLUMN tip_amount REAL DEFAULT 0`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE transactions ADD COLUMN signature_data TEXT`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE transactions ADD COLUMN card_type TEXT`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE products ADD COLUMN vendor TEXT`); } catch { /* already exists */ }
  let addedAgeCol = false;
  try { db.exec(`ALTER TABLE products ADD COLUMN age_restricted INTEGER NOT NULL DEFAULT 0`); addedAgeCol = true; } catch { /* already exists */ }
  // One-time default: mark all existing inventory as 21+ (vape shop). Runs only
  // the first time the column is added, so later per-item edits are preserved.
  if (addedAgeCol) { try { db.exec(`UPDATE products SET age_restricted = 1`); } catch { /* no-op */ } }
  try { db.exec(`ALTER TABLE customers ADD COLUMN license_number TEXT`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE customers ADD COLUMN address TEXT`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE customers ADD COLUMN city TEXT`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE customers ADD COLUMN state TEXT`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE customers ADD COLUMN zip TEXT`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE customers ADD COLUMN dob TEXT`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE receipt_config ADD COLUMN min_age INTEGER DEFAULT 21`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE customers ADD COLUMN loyalty_points INTEGER NOT NULL DEFAULT 0`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE customers ADD COLUMN lifetime_points INTEGER NOT NULL DEFAULT 0`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE transaction_items ADD COLUMN variant_id INTEGER`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE customers ADD COLUMN gold_member INTEGER NOT NULL DEFAULT 0`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE promo_codes ADD COLUMN benefit_kind TEXT`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE promo_codes ADD COLUMN benefit_value REAL DEFAULT 0`); } catch { /* already exists */ }
  try { db.exec(`ALTER TABLE promo_codes ADD COLUMN benefit_category TEXT`); } catch { /* already exists */ }
  // Lock in Gold for anyone who has ever crossed the lifetime threshold
  try { db.exec(`UPDATE customers SET gold_member = 1 WHERE lifetime_points >= 1500`); } catch { /* no-op */ }

  // Update tax rate to 8.25% if still at old default 8%
  db.exec(`UPDATE receipt_config SET tax_rate = 0.0825 WHERE id = 1 AND tax_rate = 0.08`);

  // Strip legacy SEI Vapes branding from existing installs so each shop
  // configures its own store name/address/phone under Settings.
  ...
  // MISC/open-price lines carry their own category so they show up "by department"
  try { db.exec(`ALTER TABLE transaction_items ADD COLUMN category TEXT`); } catch { /* already exists */ }
