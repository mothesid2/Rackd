...
/**
 * Manager rebate-rule CRUD (Part 2). Rules are authored on the POS by a manager
 * and written to the local `rebate_rules` table + pushed to the tenant config in
 * the cloud (enqueueRebateRule), so every register pulls them down and can detect
 * rebates offline.
 */
...
export function registerRebateHandlers(): void {
  // Manufacturers for the rule dropdown (seeded: Altria / RJR / ITG).
  ...
  // Active rules for checkout detection: is_active + within the date window.
  ...
  // Record rebates applied on a completed sale (audit trail → weekly report).
  ...
  // Record fully-qualifying rebates the cashier did NOT apply (Off mode) for
  // reconciliation / reporting — no discount is given.
