...
import { enqueueInventorySnapshot, enqueueStockMovement } from '../supabase/sync';
...
type NewProductInput = {
  ...
  age_restricted?: number; low_stock_alert?: number; is_taxable?: number;
};


export function insertProduct(db: ReturnType<typeof getDb>, product: NewProductInput): number | bigint {
  let newId: number | bigint = 0;
  db.transaction(() => {
    const result = db
      .prepare(
        `INSERT INTO products (barcode, name, category, vendor, price, cost, stock_qty, low_stock_threshold, age_restricted, low_stock_alert, is_taxable)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      )
      .run(
...
export function registerProductHandlers(): void {
  ...
  ipcMain.handle('products:add', async (_event, product: NewProductInput) => {
    try {
      const w = assertWritable('inventory_edit'); if (!w.ok) return { success: false, error: w.error };
      if (getCurrentSession()?.role !== 'manager') return { success: false, error: 'Manager access required' };
      const db = getDb();
      const newId = insertProduct(db, product);
  ...
  ipcMain.handle('products:quickAddFromSale', async (_event, product: NewProductInput) => {
    try {
      const w = assertWritable('inventory_edit'); if (!w.ok) return { success: false, error: w.error };
      if (!getCurrentSession()) return { success: false, error: 'Not signed in' };
      const db = getDb();
      const newId = insertProduct(db, product);
      return { success: true, id: newId };
    } catch (err) {
  ...
  ipcMain.handle('products:update', async (_event, id: number, data: Record<string, unknown>) => {
    try {
      ...
      if (getCurrentSession()?.role !== 'manager') return { success: false, error: 'Manager access required' };
      const db = getDb();
      const allowed = ['barcode', 'name', 'category', 'vendor', 'price', 'cost', 'stock_qty', 'low_stock_threshold', 'age_restricted', 'low_stock_alert', 'is_taxable', 'pricing_group_uid'];
      const fields = Object.keys(data).filter((k) => allowed.includes(k));
      if (fields.length === 0) return { success: false, error: 'No valid fields' };
  ...
  ipcMain.handle('products:adjustStock', async (_event, id: number, delta: number, _reason: string, override?: string) => {
    try {
      const w = assertWritable('inventory_edit'); if (!w.ok) return { success: false, error: w.error };
      const db = getDb();
      const perm = requirePermission('inventory_adjust', { override, action: 'inventory_adjust' }, db);
      ...
      db.transaction(() => {
        ...
        enqueueStockMovement(id, delta, 'manual', db);
      })();
      return { success: true };
    } catch (err) {
      return { success: false, error: String(err) };
  ...
  ipcMain.handle('products:reorderReport', async (_event, windowDays = 30) => {
    try {
      ...
      for (const r of rows) {
        ...
        const targetStock = Math.ceil(r.sold * 1.2);          
