...
/**
 * Onboarding flow (spec v2 §7). Single-store: stages are tracked in the local
 * settings table and, where possible, auto-detected from real data (products,
 * sales, staff, terminal, etc.) so the checklist reflects the actual store state
 * rather than a hand-maintained flag. Stages 7 & 8 are optional.
 */
...
interface StageDef {
  ...
  next: string; // plain-text guidance to reach this stage
}
...
export function registerOnboardingHandlers(): void {
  ipcMain.handle('onboarding:status', () => {
    try {
      ...
      const stages = STAGES.map((s) => {
        ...
        // Stamp a completion time the first time a stage is observed done.
      ...
      // Required stages = all non-optional. Complete when every required stage is done.
      ...
      // First incomplete stage drives the checklist "current" marker + next action.
      ...
      // Book-a-call nudge: they've added products but haven't rung up a sale yet.
  ...
  // Hide the checklist for good (does not reset detected stages).
