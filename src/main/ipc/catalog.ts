...
/**
 * Pre-loaded SKU catalog (spec v2 §3). Deterministic, fully local. The catalog is
 * populated from distributor invoice files via catalog:merge; on a receiving scan,
 * catalog:lookup resolves a barcode to product data (catalog-first, before manual
 * entry). Not-found scans get saved to the local (source='local') catalog so the
 * next scan hits.
 *
 * Conflict priority (highest first): local > dojo > verus > mclane > coremark.
 */
...
// ── Column mapping ──────────────────────────────────────────────────────────
// Canonical field -> the header aliases distributors commonly use. A file is
// normalized by matching its headers (case/space/underscore-insensitive) against
// these. Per-source defaults are best guesses; the UI lets the owner override the
// mapping per import, so unknown layouts still load.
...
/** Auto-map a file's headers to canonical fields using the alias table. */
...
// ── Parse an uploaded file into raw rows (array of objects) ─────────────────
...
/** Accept CSV text, JSON array text, or an already-parsed array of objects. */
...
function toCanon(raw: Record<string, unknown>, mapping: Partial<Record<CanonField, string>>): CanonRow | null {
  ...
  if (!product_name) return null; // a catalog row must at least have a name
  return {
...
export function registerCatalogHandlers(): void {
  ...
  ipcMain.handle('catalog:lookup', (_e, q: { barcode?: string; name?: string; brand?: string }) => {
    try {
      ...
      // Fuzzy: name/brand contains, needs confirmation.
  ...
  // ── Save a store-entered row to the local catalog (after a not_found scan) ──
  ...
  // ── Preview headers + suggested mapping for an upload (no writes) ──────────
  ...
  // ── Merge a distributor invoice into a source ─────────────────────────────
    ...
    try {
      ...
      const run = db.transaction(() => {
        for (const raw of rawRows) {
          ...
          // Cross-source conflict: same barcode, a DIFFERENT product name elsewhere.
          if (c.barcode) {
          ...
          if (c.barcode) {
            ...
            // No barcode — can't dedupe reliably; insert as a name-only row.
            insert.run({ source, ...c }); added++;
          }
        }
      });
  ...
  // ── Counts per source (for the admin page) ────────────────────────────────
  ...
  // ── Clear one source (re-import from scratch) ─────────────────────────────
