...
export function registerPromoHandlers(): void {
  ...
  ipcMain.handle('promos:birthdaysToday', async () => {
    try {
      ...
      const db = getDb();
      const today = DateTime.now().setZone(getBusinessTZ());
    ...
    try {
      ...
      for (const c of birthdays) {
        ...
        const benefit = isGold
          ? { kind: 'free_category', value: 0, category: 'Vape' }   
          : { kind: 'discount_amount', value: 5, category: null };  
  ...
  ipcMain.handle('promos:validate', async (_event, code: string) => {
    try {
      ...
      if (row.expires_at && DateTime.now().setZone(getBusinessTZ()).toISODate()! > row.expires_at) {
