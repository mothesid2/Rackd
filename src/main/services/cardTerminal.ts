...
// ─────────────────────────────────────────────
//  MOCK TERMINAL — for testing only
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
//  PAX TERMINAL — PacketPAX protocol over TCP
//  Supports: A920, A80, A35, S300, S500, SP30
//
//  Frame format:
//    STX (0x02)
//    LEN_HI  LEN_LO  (big-endian, length of data field)
//    DATA    (command fields separated by FS 0x1C)
//    ETX (0x03)
//    LRC     (XOR of all bytes from STX through ETX inclusive)
//
//  DoCredit (T00) sale fields (1-indexed, FS-delimited):
//    [0] Command   = "T00"
//    [1] Version   = "1.28"
//    [2] TransType = "01" (sale)
//    [3] Amount    = cents, no decimal  e.g. "1999"
//    [4] Tip       = "0"
//    [5] CashBack  = "0"
//    [6] MerchID   = "" (use terminal configured value)
//    [7] RefNum    = incrementing reference
//    [8] ZipCode   = ""
//    [9] Address   = ""
//    [10] InvNum   = invoice/order number
//    [11] STAN     = ""
//    [12] OrigRefNum = ""
//    [13] AuthCode = "" (for force-post only)
//    [14] TraceNum = ""
//    [15] CertNum  = ""
//    [16] Reserved = ""
//
//  Response fields (FS-delimited):
//    [0]  Command echo
//    [1]  Version
//    [2]  ResultCode  "000000" = approved
//    [3]  ResultText  e.g. "OK"  or "DECLINE"
//    [4]  AuthCode
//    [5]  RefNum
//    [6]  EntryMode
//    [7]  Amount (approved)
//    [8]  AppCode (EMV)
//    [9]  Statement
//    [10] CardBrand   e.g. "VISA"
//    [11] MaskedPAN   e.g. "***************1234"
//    [12] ExpiryDate
//    [13] CardHolderName
//    [14] TraceNum
//    [15] BatchNum
...
class PaxTerminal implements CardTerminal {
  ...
  sendPayment(payload: ValorPayload): Promise<TerminalResult> {
    return new Promise((resolve) => {
      ...
      // Build data payload — fields separated by FS (0x1C)
      const FS = '\x1C';
      const fields = [
        'T00',       // [0]  Command: DoCredit
        '1.28',      // [1]  Version
        '01',        // [2]  TransType: Sale
        amountCents, // [3]  Amount in cents
        '0',         // [4]  Tip
        '0',         // [5]  CashBack
        '',          // [6]  MerchantID (use terminal default)
        refNum,      // [7]  ReferenceNum
        '',          // [8]  ZipCode
        '',          // [9]  Address
        refNum,      // [10] InvoiceNum
        '',          // [11] STAN
        '',          // [12] OrigRefNum
        '',          // [13] AuthCode (force-post only)
        '',          // [14] TraceNum
        '',          // [15] CertNum
        '',          // [16] Reserved
      ];
      ...
      // LRC = XOR of every byte from STX through ETX
      let lrc = 0;
      ...
      client.on('close', () => {
        ...
        if (chunks.length > 0) {
          try {
            ...
            // already resolved via 'end' in most cases
          }
        }
      });
...
function parsePaxResponse(buf: Buffer): TerminalResult {
  // Strip STX, length bytes, ETX, LRC
  // Find STX
  ...
  // ResultCode at index 2: "000000" = approved
...
// ─────────────────────────────────────────────
//  DEJAVOO TERMINAL — ZLink API over HTTP
//  Supports: Z8, Z11, Z9, QD2, QD4
//
//  ZLink sends an HTTP POST to the terminal's
//  built-in web server.  The terminal listens on
//  port 8080 (default) at /api/v2/Pay
//
//  Request body (XML):
//    <ZLink_Packet>
//      <Request_ID>   unique ID per transaction  </Request_ID>
//      <TPDUHeader>   600000000000               </TPDUHeader>
//      <FuncCode>     1000 = Sale                </FuncCode>
//      <Amount>       in cents                   </Amount>
//      <TipAmount>    0                          </TipAmount>
//      <InvoiceNum>   your reference             </InvoiceNum>
//      <ECRRefNum>    your reference             </ECRRefNum>
//    </ZLink_Packet>
//
//  Response fields used:
//    <RespCode>       00 = approved              </RespCode>
//    <RespMsg>        "APPROVED" or decline text </RespMsg>
//    <AuthCode>       approval code              </AuthCode>
//    <RefNum>         reference from terminal    </RefNum>
//    <AcctNum>        masked PAN                 </AcctNum>
//    <CardType>       VISA / MC / AMEX / DISC    </CardType>
// ─────────────────────────────────────────────
class DejavooTerminal implements CardTerminal {
  ...
  sendPayment(payload: ValorPayload): Promise<TerminalResult> {
    ...
    const xmlBody = [
      ...
      '  <TPDUHeader>600000000000</TPDUHeader>',
      '  <FuncCode>1000</FuncCode>',       // 1000 = Credit Sale
...
function parseDejavooResponse(xml: string): TerminalResult {
  ...
  // RespCode "00" = approved; some firmware uses "000"
