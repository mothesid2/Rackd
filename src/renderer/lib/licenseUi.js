/* Rackd license banner + screen lockdown.
 * - Polls license:status every 30s (and on sync:status pushes).
 * - Fixed, full-width, non-dismissable banner: yellow at 48h grace, red at
 *   read-only lockdown, with a Retry button (license:refresh) when locked.
 * - Disables every [data-write-action] element while read-only and adds a lock
 *   icon + "Read-only — license inactive" tooltip. */
(function () {
  const POLL_MS = 30000;
  let bannerEl = null;
  ...
  function render(status) {
    ...
    if (status.readOnly) {
      ...
      btn.onclick = async () => {
        ...
        try { await window.api.licenseRefresh(); } catch (e) { /* ignore */ }
        await poll();
      };
      el.appendChild(btn);
    }
